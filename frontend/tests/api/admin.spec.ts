...
test.describe('Domain 7: Admin Operations', () => {
    ...
    test('GET /api/admin/hello → 200', async ({ request }) => {
        ...
        await assertOk('GET /api/admin/hello', res);
    });

    test('GET /api/admin/stats → 200 (cached: adminStats)', async ({ request }) => {
        const res = await request.get(`${API_BASE}/api/admin/stats`, {
            headers: authHeaders(adminToken),
        });
        await assertOk('GET /api/admin/stats', res);
    ...
    test('GET /api/homestays/all → admin all listings', async ({ request }) => {
        const res = await request.get(`${API_BASE}/api/homestays/all`, {
            headers: authHeaders(adminToken),
        });
        await assertOk('GET /api/homestays/all', res);
    });

    test('GET /api/homestays/pending → admin pending queue', async ({ request }) => {
        const res = await request.get(`${API_BASE}/api/homestays/pending`, {
            headers: authHeaders(adminToken),
        });
        await assertOk('GET /api/homestays/pending', res);
    });

    test('PUT /api/admin/homestays/{id}/feature → toggle featured', async ({ request }) => {
    ...
    test('DELETE /api/admin/posts/{id} → force delete', async ({ request }) => {
        ...
        await assertOk('DELETE /api/admin/posts/{id}', res);
    });

    test('CACHE-HIT: GET /api/admin/stats twice → no crash', async ({ request }) => {
